...
public class BMSModelUtils {
	 ...
	 * @return 総ノート数
	 */
	public static int getTotalNotes(bms.model.BMSModel model) {
		return getTotalNotes(model, 0, Integer.MAX_VALUE);
	}

	public static int getTotalNotes(bms.model.BMSModel model, int type) {
	 ...
	 */
	public static int getTotalNotes(bms.model.BMSModel model, int start, int end) {
	 ...
	 */
	public static int getTotalNotes(bms.model.BMSModel model, int start, int end, int type) {
	 ...
	 * @return 指定の時間範囲、指定の種類のの総ノート数
	 */
	public static int getTotalNotes(bms.model.BMSModel model, int start, int end, int type, int side) {
		...
		for (bms.model.TimeLine tl : model.getAllTimeLines()) {
			if (tl.getTime() >= start && tl.getTime() < end) {
				...
				case TOTALNOTES_KEY:
					for (int lane : nlane) {
						if (tl.existNote(lane) && (tl.getNote(lane) instanceof bms.model.NormalNote)) {
							count++;
						}
					}
					break;
				case TOTALNOTES_LONG_KEY:
					for (int lane : nlane) {
						if (tl.existNote(lane) && (tl.getNote(lane) instanceof bms.model.LongNote)) {
							bms.model.LongNote ln = (bms.model.LongNote) tl.getNote(lane);
							if (ln.getType() == bms.model.LongNote.TYPE_CHARGENOTE
									|| ln.getType() == bms.model.LongNote.TYPE_HELLCHARGENOTE
									|| (ln.getType() == bms.model.LongNote.TYPE_UNDEFINED && model.getLntype() != bms.model.BMSModel.LNTYPE_LONGNOTE)
									|| !ln.isEnd()) {
								count++;
							}
						}
				...
				case TOTALNOTES_LONG_SCRATCH:
					for (int lane : slane) {
						...
						if (n instanceof bms.model.LongNote) {
							final bms.model.LongNote ln = (bms.model.LongNote) n;
							if (ln.getType() == bms.model.LongNote.TYPE_CHARGENOTE
									|| ln.getType() == bms.model.LongNote.TYPE_HELLCHARGENOTE
									|| (ln.getType() == LongNote.TYPE_UNDEFINED && model.getLntype() != bms.model.BMSModel.LNTYPE_LONGNOTE)
									|| !ln.isEnd()) {
								count++;
							}
						}
				...
				case TOTALNOTES_MINE:
					for (int lane : nlane) {
						if (tl.existNote(lane) && (tl.getNote(lane) instanceof bms.model.MineNote)) {
							count++;
						}
					}
					for (int lane : slane) {
	...
	public double getAverageNotesPerTime(bms.model.BMSModel model, int start, int end) {
		return (double) this.getTotalNotes(model, start, end) * 1000 / (end - start);
	}

	public static void changeFrequency(bms.model.BMSModel model, float freq) {
	...
	public static double getMaxNotesPerTime(bms.model.BMSModel model, int range) {
		int maxnotes = 0;
		bms.model.TimeLine[] tl = model.getAllTimeLines();
	...
	public static long setStartNoteTime(BMSModel model, long starttime) {
		...
		if(marginTime > 0) {
			...
			bms.model.TimeLine[] tl2 = new bms.model.TimeLine[model.getAllTimeLines().length + 1];
