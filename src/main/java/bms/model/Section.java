...
public class Section {
	...
	public Section(BMSModel model, Section prev, List<String> lines, Map<Integer, Double> bpmtable,
		...
		for (String line : lines) {
			...
			// 5/10KEY -> 7/14KEY
			if(ch2 == 7 || ch2 == 8) {
				final bms.model.Mode mode = (model.getMode() == bms.model.Mode.BEAT_5K) ? bms.model.Mode.BEAT_7K : (model.getMode() == bms.model.Mode.BEAT_10K ? bms.model.Mode.BEAT_14K : null);
				if(mode != null) {
					this.processData(line, (pos, data) -> {
						model.setMode(mode);
					});
			...
			if(basech == P2_KEY_BASE || basech == P2_INVISIBLE_KEY_BASE || basech == P2_LONG_KEY_BASE || basech ==P2_MINE_KEY_BASE) {
				final bms.model.Mode mode = (model.getMode() == bms.model.Mode.BEAT_5K) ? bms.model.Mode.BEAT_10K : (model.getMode() == bms.model.Mode.BEAT_7K ? bms.model.Mode.BEAT_14K : null);
				if(mode != null) {
					this.processData(line, (pos, data) -> {
						model.setMode(mode);
					});
	...
	public void makeTimeLines(int[] wavmap, int[] bgamap, TreeMap<Double, TimeLineCache> tlcache, List<LongNote>[] lnlist, LongNote[] startln) {
		...
		final int[] cassign = model.getMode() == bms.model.Mode.POPN_9K ? CHANNELASSIGN_POPN :
                              (model.getMode() == bms.model.Mode.BEAT_7K || model.getMode() == Mode.BEAT_14K ? CHANNELASSIGN_BEAT7 : CHANNELASSIGN_BEAT5);
		// 小節線追加
		final TimeLine basetl = getTimeLine(sectionnum);
		basetl.setSectionLine(true);
		
		if(poor.length > 0) {
			final bms.model.Layer.Sequence[] poors = new bms.model.Layer.Sequence[poor.length + 1];
			final int poortime = 500;
			
			for (int i = 0; i < poor.length; i++) {
				if (bgamap[poor[i]] != -2) {
					poors[i] = new bms.model.Layer.Sequence((long)(i * poortime / poor.length), bgamap[poor[i]]);
				} else {
					poors[i] = new bms.model.Layer.Sequence((long)(i * poortime / poor.length), -1);
				}
			}
			poors[poors.length - 1] = new bms.model.Layer.Sequence(poortime);
			basetl.setEventlayer(new bms.model.Layer[] {new bms.model.Layer(new bms.model.Layer.Event(EventType.MISS, 1), new Layer.Sequence[][] {poors})});
		}
					...
					if (data == lnobj) {
						...
						for (Map.Entry<Double, TimeLineCache> e : tlcache.descendingMap().entrySet()) {
							...
							if (tl2.existNote(key)) {
								final bms.model.Note note = tl2.getNote(key);
			...
			case P1_LONG_KEY_BASE:
				this.processData(line, (pos, data) -> {
					...
					if(!insideln) {
						...
						if (startln[key] == null) {
							if(tl.existNote(key)) {
								bms.model.Note note = tl.getNote(key);
							...
							for (Map.Entry<Double, TimeLineCache> e : tlcache.descendingMap().entrySet()) {
								...
								if(tl2.getSection() == startln[key].getSection()) {
									bms.model.Note note = startln[key];
